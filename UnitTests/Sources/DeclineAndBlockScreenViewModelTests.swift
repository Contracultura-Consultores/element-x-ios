...
class DeclineAndBlockScreenViewModelTests: XCTestCase {
    ...
    override func setUp() {
        ...
        viewModel = DeclineAndBlockScreenViewModel(userID: "@alice:matrix.pesbc.chat",
    ...
    func testDeclineBlockAndReport() async throws {
        ...
        clientProxy.roomForIdentifierClosure = { id in
            XCTAssertEqual(id, "!room:matrix.pesbc.chat")
        ...
        clientProxy.ignoreUserClosure = { userId in
            XCTAssertEqual(userId, "@alice:matrix.pesbc.chat")
            return .success(())
        }
