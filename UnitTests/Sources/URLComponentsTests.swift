...
class URLComponentsTests: XCTestCase {
    func testAddFragmentQueryItems() {
        guard let url = URL(string: "https://test.matrix.pesbc.chat"),
        ...
        XCTAssertEqual(components.url?.absoluteString, "https://test.matrix.pesbc.chat#?first=1&second=2")
    }
    
    func testRemoveFragmentQueryItem() {
        guard let url = URL(string: "https://test.matrix.pesbc.chat#random/data?first=1&second=2"),
              var components = URLComponents(url: url, resolvingAgainstBaseURL: true) else {
            XCTFail("URL invalid")
            return
        }
        ...
        XCTAssertEqual(components.url?.absoluteString, "https://test.matrix.pesbc.chat#random/data?second=2")
    }
    ...
    func testAppendFragmentQueryItem() {
        ...
        XCTAssertEqual(components.url?.absoluteString, "https://test.matrix.pesbc.chat#/random/data?first=1&mr%20in%20between=hello&second=2")
    }
    ...
    func testChangeFragmentQueryItemValue() {
        ...
        XCTAssertEqual(components.url?.absoluteString, "https://test.matrix.pesbc.chat#/random/data?first=last&second=2")
    }
