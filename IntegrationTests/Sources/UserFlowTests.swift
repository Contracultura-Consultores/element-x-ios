...
class UserFlowTests: XCTestCase {
    private static let integrationTestsRoomName = "PESenger iOS Integration Tests"
    ...
    private func checkRoomFlows() {
        ...
        // won't be found
        // Remove once https://github.com/Contracultura-Consultores/element-x-ios/issues/3365 gets sorted
        sleep(30)
    ...
    private func checkTimelineItemActionMenu() {
        ...
        let timelineItemActionMenu = app.scrollViews[
            A11yIdentifiers.roomScreen.timelineItemActionMenu
        ].firstMatch
    ...
    private func tapOnBackButton(_ label: String = "Back") {
        let button = app.buttons.matching(
            NSPredicate(format: "label == %@ && identifier == ''", label)
        ).firstMatch
        XCTAssertTrue(button.waitForExistence(timeout: 10.0))
        button.tapCenter()
    }
