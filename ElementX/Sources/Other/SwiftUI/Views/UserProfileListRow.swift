...
struct UserProfileCell_Previews: PreviewProvider, TestablePreview {
    ...
    static var previews: some View {
        Form {
            ...
            UserProfileListRow(user: .init(userID: "@someone:matrix.pesbc.chat"), membership: .join, mediaProvider: MediaProviderMock(configuration: .init()),
                               kind: .multiSelection(isSelected: false, action: action))
                .disabled(true)
            
            UserProfileListRow(user: .init(userID: "@someone:matrix.pesbc.chat"), membership: nil, mediaProvider: MediaProviderMock(configuration: .init()),
                               kind: .multiSelection(isSelected: false, action: action))
        }
        .compoundList()
    }
