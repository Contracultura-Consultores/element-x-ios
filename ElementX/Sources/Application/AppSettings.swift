...
final class AppSettings {
    ...
    static func resetSessionSpecificSettings() {
        ...
        store.removeObject(
            forKey: UserDefaultsKeys.hasRunIdentityConfirmationOnboarding.rawValue)
    }
    ...
    static var isDevelopmentBuild: Bool = {
        ...
        true
        #else
        let apps = ["io.pesbc.pesenger.nightly", "io.pesbc.pesenger.pr"]
    ...
    private(set) var defaultHomeserverAddress = "matrix.pesbc.chat"

    /// The task identifier used for background app refresh. Also used in main target's the Info.plist
    let backgroundAppRefreshTaskIdentifier = "io.pesbc.pesenger.background.refresh"
    ...
    private(set) var identityPinningViolationDetailsURL: URL =
        "https://element.io/help#encryption18"
    /// Any domains that Element web may be hosted on - used for handling links.
    private(set) var elementWebHosts = [
        "app.element.io", "staging.element.io", "develop.element.io"
    ]

    @UserPreference(key: UserDefaultsKeys.appAppearance, defaultValue: .system,
    ...
    /// The redirect URL used for OIDC. This no longer uses universal links so we don't need the bundle ID to avoid conflicts between PESenger, Nightly and PR builds.
    ...
    private(set) var pushGatewayBaseURL: URL = "https://matrix.pesbc.chat"
    ...
    let analyticsConfiguration: AnalyticsConfiguration? =
        AppSettings.makeAnalyticsConfiguration()
    ...
    private static func makeAnalyticsConfiguration() -> AnalyticsConfiguration? {
        guard let host = Secrets.postHogHost, let apiKey = Secrets.postHogAPIKey else {
            return nil
        }
    ...
    @UserPreference(key: UserDefaultsKeys.analyticsConsentState,
                    defaultValue: AnalyticsConsentState.unknown, storageType: .userDefaults(store))
    ...
    @UserPreference(key: UserDefaultsKeys.sharePresence, defaultValue: true,
                    storageType: .userDefaults(store))
    var sharePresence
    ...
    @UserPreference(key: UserDefaultsKeys.knockingEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var knockingEnabled

    @UserPreference(key: UserDefaultsKeys.reportRoomEnabled, defaultValue: false,
                    storageType: .userDefaults(store)) var reportRoomEnabled

    @UserPreference(key: UserDefaultsKeys.reportInviteEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var reportInviteEnabled

    @UserPreference(key: UserDefaultsKeys.threadsEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var threadsEnabled
    ...
    @UserPreference(key: UserDefaultsKeys.logLevel, defaultValue: LogLevel.info,
                    storageType: .userDefaults(store))
    var logLevel
    ...
    @UserPreference(key: UserDefaultsKeys.hideInviteAvatars, defaultValue: false,
                    storageType: .userDefaults(store))
    var hideInviteAvatars

    @UserPreference(key: UserDefaultsKeys.timelineMediaVisibility, defaultValue: TimelineMediaVisibility.always,
                    storageType: .userDefaults(store))
    var timelineMediaVisibility

    @UserPreference(key: UserDefaultsKeys.isNewBloomEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
